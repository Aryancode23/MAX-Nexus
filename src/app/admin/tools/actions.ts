...
export async function createTool(formData: FormData): Promise<{ error?: string }> {
  ...
  await logActivity("Added tool", "tool", values.name);
...
export async function updateTool(id: string, formData: FormData): Promise<{ error?: string }> {
  ...
  await logActivity("Edited tool", "tool", values.name);
  revalidatePath("/admin/tools");
  revalidatePath("/tools");
  revalidatePath("/");
...
export async function setToolStatus(id: string, status: "active" | "disabled" | "coming_soon") {
  const supabase = createClient();
  const { data } = await supabase.from("tools").select("name").eq("id", id).maybeSingle();
  await supabase.from("tools").update({ status }).eq("id", id);
  await logActivity(status === "disabled" ? "Disabled tool" : status === "active" ? "Enabled tool" : "Set tool to coming soon", "tool", data?.name);
  revalidatePath("/admin/tools");
  revalidatePath("/tools");
  revalidatePath("/");
}
...
export async function deleteTool(id: string) {
  ...
  await logActivity("Deleted tool", "tool", data?.name);
  revalidatePath("/admin/tools");
  revalidatePath("/tools");
  revalidatePath("/");
}
